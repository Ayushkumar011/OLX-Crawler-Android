...
import {
  ...
  useUpdateNegativeKeyword,
  useDeleteNegativeKeyword,
  getGetNegativeKeywordsQueryKey
...
export default function Home() {
  ...
  const { data: storedKeywords, isLoading: loadingKeywords } = useGetNegativeKeywords();
  const addKeywordMutation = useAddNegativeKeyword();
  const updateKeywordMutation = useUpdateNegativeKeyword();
  const deleteKeywordMutation = useDeleteNegativeKeyword();

  const handleAddKeyword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newKeywordValue.trim()) return;
    try {
      await addKeywordMutation.mutateAsync({ data: { keyword: newKeywordValue.trim() } });
      setNewKeywordValue("");
      queryClient.invalidateQueries({ queryKey: getGetNegativeKeywordsQueryKey() });
  ...
  const handleUpdateKeyword = async (id: number) => {
    ...
    try {
      await updateKeywordMutation.mutateAsync({ id, data: { keyword: editKeywordValue.trim() } });
  ...
  const handleCopyKeyword = (keyword: string) => {
    ...
    const current = negativeKeywords.split(',').map(k => k.trim()).filter(Boolean);
    if (!current.includes(keyword)) {
      current.push(keyword);
      setNegativeKeywords(current.join(', '));
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          >
            ...
            {/* Saved Negative Keywords Section */}
            <GlassCard>
              ...
              <form onSubmit={handleAddKeyword} className="flex gap-2 mb-4">
                <GlassInput
                  ...
                  className="flex-1"
                />
                <NeonButton type="submit" className="p-2 px-4 flex items-center shrink-0">
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                {loadingKeywords ? (
                  ...
                  <div className="text-center text-white/50 py-4 border border-dashed border-white/10 rounded-lg">
                      ...
                      {editingKeywordId === kw.id ? (
                        <div className="flex flex-1 items-center gap-2">
                          <GlassInput
                            ...
                            autoFocus
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') handleUpdateKeyword(kw.id);
